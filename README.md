...
- `스페이스` / `위쪽 화살표`: 날갯짓
- 캔버스 클릭 / 터치: 날갯짓
- `A` 또는 `피자 모드` 버튼: 피자 모드 전환. 실제 날갯짓 입력을 자동으로 넣어 관상용으로 게임을 진행합니다.
- `M`: 음소거 전환
...
`npm run verify`는 린트, 타입 검사, 단위 테스트, 프로덕션 빌드, 브라우저 E2E를 모두 실행합니다. `npm run actual-play`는 실제 브라우저 입력으로 여러 번 플레이하며 모든 기믹 관측, 재시작 안정성, 피자 모드 진행, 콘솔 오류 유무를 확인합니다.

## 테스트용 디버그 표면

앱은 재현 가능한 테스트만을 위해 `window.crappyDebug.snapshot()`과 `window.crappyDebug.setSeed(seed)`를 제공합니다. 피자 모드는 공개 디버그 명령이 아니라 UI 기능이며 기존 `flap()` 입력 경로를 사용합니다. 점수 지급, 위험 제거, 생존 강제, 충돌 비활성화, 순간 이동, 자동 통과 같은 치트성 명령은 제공하지 않습니다.
